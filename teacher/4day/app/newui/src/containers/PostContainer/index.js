...
//import {getComments, getMyPost} from '../../services'
// 다른 컴포넌트 가져오기
...
class PostContainer extends Component
{
 ...
 componentWillMount()
 {
  // 렌더가 호출되기 전단계 -> 네트워크 가동
  // 1글을 기본글로 해서 글을 가져온다(1번글과 1번글댓글)
 ...
 render () {
  ...
  return (
   <PostWrapper>
    {/* 네비게이터 (이전글, 다음글) */}
    <Navigate postId={postId} fetching={fetching} 
              onClick={this.postRequestClick} />
    {/* 포스팅 (제목, 내용, 댓글리스트) */}
    <Post postId={postId} 
          title={post.title}
          body={post.body}
